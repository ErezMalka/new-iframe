...
export class MyBenefitsComponent extends SizeMobileInitializationComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  public stringedDateFomat(stringedDate) {
   ...
   displayPizzaLogic(): void {
    //pizza logic
    this.myOrder.OrderPizzas.forEach((orderPizza) => {
      orderPizza.quarter1 = [];
      orderPizza.quarter2 = [];
      orderPizza.quarter3 = [];
      orderPizza.quarter4 = [];
      ...
      orderPizza.quarter2 = orderPizza.Toppings.filter((e) => { return e.Quarter2 == true; })
        .map((t) => { return t.Topping.Name });
      orderPizza.quarter3 = orderPizza.Toppings.filter((e) => { return e.Quarter3 == true; })
        .map((t) => { return t.Topping.Name });
      orderPizza.quarter4 = orderPizza.Toppings.filter((e) => { return e.Quarter4 == true; })
        .map((t) => { return t.Topping.Name });

      orderPizza.Halfs = [];
      orderPizza.Full = [];

      if (orderPizza.quarter1.length > 0 &&
        JSON.stringify(orderPizza.quarter1) == JSON.stringify(orderPizza.quarter2) &&
        JSON.stringify(orderPizza.quarter1) == JSON.stringify(orderPizza.quarter3) &&
        JSON.stringify(orderPizza.quarter1) == JSON.stringify(orderPizza.quarter4)) {
        orderPizza.Full.concat(orderPizza.quarter1);
      }
      else {
        if (orderPizza.quarter1.length > 0 &&
          JSON.stringify(orderPizza.quarter1) == JSON.stringify(orderPizza.quarter2)) {
          orderPizza.Halfs.push(orderPizza.quarter1);
          orderPizza.quarter1 = orderPizza.quarter2 = [];
        }
        if (orderPizza.quarter1.length > 0 &&
          JSON.stringify(orderPizza.quarter1) == JSON.stringify(orderPizza.quarter3)) {
          orderPizza.Halfs.push(orderPizza.quarter1);
          orderPizza.quarter1 = orderPizza.quarter3 = [];
        }
        if (orderPizza.quarter1.length > 0 &&
          JSON.stringify(orderPizza.quarter1) == JSON.stringify(orderPizza.quarter4)) {
          orderPizza.Halfs.push(orderPizza.quarter1);
          orderPizza.quarter1 = orderPizza.quarter4 = [];
        }
        if (orderPizza.quarter2.length > 0 &&
          JSON.stringify(orderPizza.quarter2) == JSON.stringify(orderPizza.quarter3)) {
          orderPizza.Halfs.push(orderPizza.quarter2);
          orderPizza.quarter2 = orderPizza.quarter3 = [];
        }
        if (orderPizza.quarter2.length > 0 &&
          JSON.stringify(orderPizza.quarter2) == JSON.stringify(orderPizza.quarter4)) {
          orderPizza.Halfs.push(orderPizza.quarter2);
          orderPizza.quarter2 = orderPizza.quarter4 = [];
        }
        if (orderPizza.quarter3.length > 0 &&
          JSON.stringify(orderPizza.quarter3) == JSON.stringify(orderPizza.quarter4)) {
          orderPizza.Halfs.push(orderPizza.quarter3);
          orderPizza.quarter3 = orderPizza.quarter4 = [];
        }
      }
    });
    /////////////
  ...
  displayPizzaLogicForOrders(): void {
    //pizza logic
    this.myOrders.forEach((order)=>{
  ...
  getPaymentType(order): string {
    if (order.PaymentTypeId == 6) {
      return this.translationsService.translate('CREDIT');
    } //else if (order.PaymentType !== 'Unknown') {
      //return "";
    //} 
    else {
      return order.PaymentTypeStr;
    }
  }
  ...
  if(this.isMobileMode()){
    ...
    const matDialogRef = this.matDialog.open(DialogSignInComponent, {
      ...
      minWidth: '346px',
      position: position,
      panelClass: ['padding-small-container', 'custom-mat-dialog'],
      disableClose: true,
    });
